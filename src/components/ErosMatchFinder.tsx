...
interface ErosMatchFinderProps {
  userProfile: any
  onFindMatches: (matches: any[]) => void
}

export default function ErosMatchFinder({ userProfile, onFindMatches }: ErosMatchFinderProps) {
  ...
  return (
    <div className="glass-bubble p-6 rounded-2xl">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-white text-xl font-bold">EROS Match Finder</h3>
          <p className="text-white/60 text-sm">Powered by Perplexity</p>
        </div>
      </div>
      
      <p className="text-white/80 mb-4">
        Let EROS analyze your profile and find the most compatible matches based on your preferences!
      </p>
      ...
      >
        {isFinding ? (
          <>
            ...
            <span>EROS is finding matches...</span>
          </>
        ) : (
          <>
            <span>🏹</span>
      ...
      {matches.length > 0 && (
        <div className="mt-6 space-y-4">
          <h4 className="text-white font-semibold">EROS Found {matches.length} Compatible Matches:</h4>
          {matches.map((match) => (
