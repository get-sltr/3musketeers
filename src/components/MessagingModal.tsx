...
export default function MessagingModal({ 
  ...
  // Get current user ID once when modal is opened
  useEffect(() => {
    if (isOpen) {
  ...
  useEffect(() => {
    // Wait for all required data: modal open, other user, and current user
    if (!isOpen || !user || !currentUserId) return
    ...
    initConversation()
  }, [isOpen, user, currentUserId, supabase])
  ...
  // Listen for new messages via Global Event (from useRealtime hook)
  useEffect(() => {
  ...
  // Memoized function to send a message
  const sendMessage = useCallback(async (e: React.FormEvent) => {
    e.preventDefault()
    // Guard clause now includes currentUserId
    if (!newMessage.trim() || !conversationId || sending || !currentUserId || !user) return
    ...
    try {
      ...
      if (isConnected && realtimeSendMessage) {
        ...
        // Fallback: insert directly and get the new row back
        ...
        if (newDbMessage) {
          // Normalize and add to state directly
          const normalized: Message = {
            id: newDbMessage.id,
            sender_id: newDbMessage.sender_id,
            receiver_id: newDbMessage.receiver_id,
            content: newDbMessage.content,
            created_at: newDbMessage.created_at,
            read: newDbMessage.read || false,
          }
  ...
  return (
    <>
          ...
          <div className="p-4 border-t border-white/10">
            <form onSubmit={sendMessage} className="flex gap-3">
              <input
                ...
                disabled={sending || !conversationId || !currentUserId}
              />
              <button
                type="submit"
                disabled={!newMessage.trim() || sending || !conversationId || !currentUserId}
      ...
      {/* Video Call Modal - NOW PASSING currentUserId ✅ */}
